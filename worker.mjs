...
function inline(buttons) { return { inline_keyboard: buttons }; }
function mainMenu() {
  return inline([
...
function contactKeyboard() {
  return { keyboard: [[{ text: "Поделиться номером", request_contact: true }], ["Моя запись"]], resize_keyboard: true, one_time_keyboard: true };
}
function removeKeyboard() { return { remove_keyboard: true }; }
function serviceMenu() {
  return inline([
    ...services.map((service) => [{ text: `${service.title} · ${service.duration}`, callback_data: `service:${service.id}` }]),
...
function freeDates() {
  ...
  for (let offset = 1; dates.length < 5; offset += 1) {
    ...
    if (date.getDay() !== 0) dates.push(isoDate(date));
  }
  return dates;
}
function dateMenu() {
  return inline([...freeDates().map((date) => [{ text: formatDate(date), callback_data: `date:${date}` }]), [{ text: "Назад к услугам", callback_data: "book" }]]);
}
function timeMenu() {
  return inline([...timeSlots.map((slot) => [{ text: slot, callback_data: `time:${slot}` }]), [{ text: "Назад к датам", callback_data: "dates" }]]);
}
function formatDate(value) {
  const [year, month, day] = value.split("-").map(Number);
  const date = new Date(year, month - 1, day, 12, 0, 0);
  return new Intl.DateTimeFormat("ru-RU", { weekday: "short", day: "2-digit", month: "long" }).format(date);
}
...
function selectedService(data) { return services.find((service) => service.id === data.serviceId); }
function appointmentId() { return `${Date.now()}-${Math.random().toString(16).slice(2, 8)}`; }
...
async function send(chatId, text, options = {}) {
  return tg("sendMessage", { chat_id: chatId, text, parse_mode: "HTML", disable_web_page_preview: false, ...options });
}
async function answer(id) { return tg("answerCallbackQuery", { callback_query_id: id }); }
async function readAppointments(env) {
  const raw = await env.APPOINTMENTS.get("items");
  return raw ? JSON.parse(raw) : [];
}
async function writeAppointments(env, items) { await env.APPOINTMENTS.put("items", JSON.stringify(items, null, 2)); }
...
function appointmentText(item) {
  return [`Услуга: ${item.service}`, `Дата и время: ${formatDate(item.date)}, ${item.time}`, `Имя: ${item.name}`, `Телефон: ${item.phone}`].join("\n");
}

async function home(chatId) {
  resetSession(chatId);
  await send(chatId, ["Здравствуйте! Это демо-бот салона красоты.", "", "Здесь клиент может посмотреть работы, выбрать услугу, дату, время и оставить номер для подтверждения.", "", "Кнопка Моя запись показывает активную запись и позволяет отменить ее с указанием причины."].join("\n"), { reply_markup: mainMenu() });
}
async function examples(chatId) {
  await send(chatId, ["Примеры работ", "", ...workExamples.map(([title, link], index) => `${index + 1}. <a href="${link}">${title}</a>`), "", "В реальном боте сюда ставятся фото салона и работ мастеров."].join("\n"), { reply_markup: inline([[{ text: "Хочу записаться", callback_data: "book" }], [{ text: "Назад", callback_data: "home" }]]) });
}
async function prices(chatId) {
  await send(chatId, ["Демо-услуги", "", ...services.map((item) => `${item.title}: ${item.price}`)].join("\n"), { reply_markup: inline([[{ text: "Записаться", callback_data: "book" }], [{ text: "Назад", callback_data: "home" }]]) });
}
async function showMine(env, chatId) {
  resetSession(chatId);
  const items = await activeAppointments(env, chatId);
  if (items.length === 0) {
    await send(chatId, "У вас пока нет активной записи.", { reply_markup: inline([[{ text: "Записаться", callback_data: "book" }], [{ text: "Назад", callback_data: "home" }]]) });
    return;
  }
  await send(chatId, ["Ваша активная запись", "", ...items.map((item, index) => `${index + 1}. ${appointmentText(item)}`)].join("\n\n"), { reply_markup: inline([...items.map((item) => [{ text: `Отменить: ${formatDate(item.date)}, ${item.time}`, callback_data: `cancel:${item.id}` }]), [{ text: "Назад", callback_data: "home" }]]) });
}
...
async function chooseService(chatId, serviceId) {
  ...
  await send(chatId, `Выбрали: ${selectedService(data).title}.\nТеперь выберите свободную дату:`, { reply_markup: dateMenu() });
}
...
async function chooseTime(chatId, time) {
  ...
  data.step = "name";
  data.time = time;
  await send(chatId, ["Почти готово.", "", `Услуга: ${selectedService(data).title}`, `Дата: ${formatDate(data.date)}`, `Время: ${time}`, "", "Как к вам обращаться?"].join("\n"));
}
...
async function receiveContact(env, chatId, contact) {
  ...
  const item = { id: appointmentId(), chatId, name: data.name, phone: contact.phone_number, telegramUserId: contact.user_id ?? null, service: service.title, date: data.date, time: data.time, status: "active", createdAt: new Date().toISOString() };
  const items = await readAppointments(env);
  items.push(item);
  await writeAppointments(env, items);
  resetSession(chatId);
  await send(chatId, ["Запись принята!", "", appointmentText(item)].join("\n"), { reply_markup: removeKeyboard() });
  await send(chatId, "Теперь запись можно посмотреть или отменить через кнопку Моя запись.", { reply_markup: mainMenu() });
}
...
async function cancel(env, chatId, reason) {
  const data = session(chatId);
  const items = await readAppointments(env);
  ...
  await writeAppointments(env, items);
  resetSession(chatId);
  await send(chatId, `Запись отменена.\n\nПричина: ${item.cancellationReason}`, { reply_markup: mainMenu() });
}
async function message(env, update) {
  const chatId = update.chat.id;
  const text = update.text?.trim();
  const data = session(chatId);
  if (text === "/start" || /^start$/i.test(text ?? "")) return home(chatId);
  if (text === "/my" || /^моя запись$/i.test(text ?? "")) return showMine(env, chatId);
  if (update.contact) return receiveContact(env, chatId, update.contact);
  if (data.step === "name" && text) return receiveName(chatId, text);
  if (data.step === "cancel_reason" && text) return cancel(env, chatId, text);
...
async function callback(env, query) {
  ...
  if (data.startsWith("cancel:")) return askCancelReason(env, chatId, data.split(":")[1]);
}
async function handleUpdate(env, update) {
  if (update.message) await message(env, update.message);
  if (update.callback_query) await callback(env, update.callback_query);
}

export default {
  async fetch(request, env) {
    if (request.method === "GET") return new Response("Beauty demo bot webhook is running.");
